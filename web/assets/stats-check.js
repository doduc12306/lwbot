...
const loginTimer = setTimeout(async () => window.location.reload(false), 5000);
...
checkClient.on('ready', async () => {
  ...
  setInterval(async () => {
    const lwbot = checkClient.users.get('377205339323367425');

    const online_indicator = document.getElementById('online-indicator');
    const online_text = document.getElementById('online-text');
    const playing_type = document.getElementById('playing-type');
    const playing_game = document.getElementById('playing-game');

    const playing_parent_ele = document.getElementById('playing-parent-ele');
    const playing_game_ele = document.getElementById('playing-game-ele');
    ...
    if(lwbot.presence.status === 'online' || lwbot.presence.status === 'idle' || lwbot.presence.status === 'dnd') {
      ...
      const etr = document.getElementById('cont1');
      ...
      const colSm4_1 = document.createElement('div');
      colSm4_1.className = 'col-sm-4';
      await row.appendChild(colSm4_1);

      const colSm4_2 = document.createElement('div');
      ...
      const colSm4_2_h1 = document.createElement('h1');
      ...
      const colSm4_3 = document.createElement('div');
