...
module.exports.run = async (client, message, args) => {
  ...
  await message.guild.modbase.create({
    ...
    let dmMsg = `${bhEmote} **You were banned from** \`${message.guild.name}\` \`|\` :bust_in_silhouette: **Responsible Moderator:** ${message.author.toString()} (${message.author.tag})`;
    ...
    await message.guild.settings.get('modLogChannel')
      .then(async modLogChannel => {
        ...
        if (modLogChannel === null) return message.channel.send(`:warning: **Ban completed, but there is no mod log channel set.** Try \`${await message.guild.settings.get('prefix')}set <edit/add> modLogChannel <channel name>\``);
