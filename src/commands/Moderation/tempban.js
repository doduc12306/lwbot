  ...
  await message.guild.modbase.create({
    ...
    await message.guild.settings.get('modLogChannel')
      .then(async modLogChannel => {
        ...
        if (!modLogChannel) return message.send(`:warning: **Tempban issued, but there is no mod log channel set.** Try \`${await message.guild.settings.get('prefix')}set <edit/add> modLogChannel <channel name>\``);
    ...
    setTimeout(async () => {
      await message.guild.modbase.create({
        ...
        await message.guild.settings.get('modLogChannel')
          .then(async modLogChannel => {
            modLogChannel = message.guild.channels.find(g => g.name.toLowerCase() === modLogChannel.toLowerCase());
            if (!modLogChannel) return message.send(`:warning: **A tempban has completed, but there is no mod log channel set.** Try \`${await message.guild.settings.get('prefix')}set <edit/add> modLogChannel <channel name>\``);
            if (!message.guild.me.permissionsIn(modLogChannel).serialize()['SEND_MESSAGES'] || !message.guild.me.permissionsIn(modLogChannel).serialize()['EMBED_LINKS']) {
