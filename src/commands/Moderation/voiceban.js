  ...
  await message.guild.modbase.create({
    ...
    await settings.get(message.guild.id, 'modLogChannel')
      .then(async modLogChannel => {
        ...
        if (!modLogChannel) return message.send(`⚠️ **Voiceban completed, but there is no mod log channel set.** Try \`${await settings.get(message.guild.id, 'prefix')}set <edit/add> modLogChannel <channel name>\``);
