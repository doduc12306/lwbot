...
if (process.argv.includes('-d') || process.argv.includes('--debug')) client.login(config.debugtoken);
...
client.on('message', message => {
  ...
  if (message.guild.id !== '381192127050153993') return message.channel.send(':x: `|` **This guild is not authorized to use any of these commands!**');

  if(message.content === `${prefix}ping`) return message.channel.send(`:ping_pong: ${Math.round(client.ping)}ms`);

  if (message.content.startsWith(`${prefix}kill`)) {
    if (!['420220760066490379', '107599228900999168'].includes(message.author.id)) return message.channel.send(':x: `|` **Only chair can access this command!**');
...
/* client.on('guildMemberAdd', member => {
  if(member.guild.id !== '381192127050153993' || member.guild.id !== '444250305618509824') return;

  if(member.displayName.match(/.gg\//gm)) message.guild.ban(member.user, {reason: '[Auto Ban] Detected .gg in username'});
}); */